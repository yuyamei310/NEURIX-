...
export function enrichArchetypeResult(result: Partial<ArchetypeResult>, fallbackArchetype: Archetype): ArchetypeResult {
  ...
  return {
    ...
    cluster_size: publicCluster?.count ?? result.cluster_size ?? profile.nodeCount,
    olympic_count: publicCluster?.olympic_count ?? result.olympic_count ?? profile.olympicSignals,
    paralympic_count: publicCluster?.paralympic_count ?? result.paralympic_count ?? profile.paralympicSignals,
    time_span: publicCluster?.time_span ?? result.time_span ?? profile.timeSpan,
    synthetic_archive_id: publicCluster?.archive_id ?? result.synthetic_archive_id ?? profile.id,
    archive_basis: publicCluster?.basis ?? result.archive_basis ?? profile.basis,
